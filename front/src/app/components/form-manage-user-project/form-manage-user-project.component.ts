...
export class FormManageUserProjectComponent {
  ...
  addUserToProject() {
    ...
    if (this.userForm.valid) {
      ...
      //On vérifie que le rôle est valide et on le convertit en français
      ...
      this.projectService
        ...
        .subscribe({
          next: (response) => {
            //Utilisateur ajouté avec succès, on reinitialise le formulaire
            //et on envoie un event pour rafraichir la liste
          ...
          error: (error) => {
            //En fonction de l'erreur, on affiche un message d'erreur
            switch (error.status) {
              case 401:
  ...
  isRoleAvailable(role: string): boolean {
    ...
    return (
      role === "Administrateur" || role === "Membre" || role === "Observateur"
    );
  }

  convertRoleToFrench(role: string): string {
    //On convertit le rôle en français
    switch (role) {
      case "admin":
