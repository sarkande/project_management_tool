...
export class FormTaskComponent {
  ...
  //Getter pour eviter les problemes de compilation avec une priorité null
  ...
  onSubmit() {
    if (this.taskForm.valid) {
      ...
      if (dueDate != null && dueDate != "") data.dueDate = new Date(dueDate);

      this.taskService.postTask(this.projectId, data).subscribe(
        (task) => {
          //On reset le formulaire et on préviens le parent de rafrachir la liste
          this.taskForm.reset();
